...
constexpr int kMmqTileYK =
    kMmqTileNeK + kMmqTileNeK / 32; // + per-32 scale floats
constexpr int kMmqMmaTileXKQ6K = 75;
...
LoadTilesQ6KMma(const char *__restrict__ x, int *__restrict__ x_tile,
  ...
  int *x_qs = x_tile;
  float *x_df = reinterpret_cast<float *>(x_qs + mmq_y * kMmqMmaTileXKQ6K);
  int *x_sc = reinterpret_cast<int *>(x_df + mmq_y);

  constexpr int QI6 = 128; // QK_K/2
  // threads_per_row = MMQ_ITER_K / (4 * QR6_K) = 256/8 = 32: one warp per
  // weight row per iteration.
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += nrows * kMmqWarps) {
    const int i =
        i0 + (nrows == 1 ? threadIdx.y : threadIdx.y * nrows +
                                             threadIdx.x / threads_per_row);
    if (i > i_max) {
      break; // need_check semantics handled by caller bounds
    }

    const block_q6_k *bxi =
        reinterpret_cast<const block_q6_k *>(x) + kbx0 + i * stride;

    // get_int_b2: 4 bytes from a 2-byte-aligned base (ql is byte offset
    // 0 of the block; qh at 128).
    const int ql = LoadPackedInt32Unaligned(&bxi->ql[2 * txi % 128]);
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += kMmqWarps * warp_size) {
    ...
    const block_q6_k *bxi =
        reinterpret_cast<const block_q6_k *>(x) + kbx0 + i * stride;
    x_df[i] = __half2float(*reinterpret_cast<const half *>(&bxi->d));
  }

  constexpr int rows_per_warp = warp_size / 4; // 8
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += kMmqWarps * rows_per_warp) {
    ...
    // get_int_b2 on the scales (2-byte aligned, 16 int8 scales packed).
...
VecDotQ6KQ8_1Mma(const int *__restrict__ x, const int *__restrict__ y,
  ...
  using TileB = m::Tile<16, 8, int>;
  using TileC = m::Tile<16, 16, int, m::kJMajor>;
  using TileLoad = m::Tile<64, 2, int>;

  constexpr int rows_per_warp = 32; // granularity for mmq_x=16
  constexpr int ntx = rows_per_warp / TileC::I; // 2

  y += (threadIdx.y % ntx) * (TileC::J * kMmqTileYK);

  const int *x_qs = x;
  const float *x_df = reinterpret_cast<const float *>(x_qs + kMmqY * kMmqMmaTileXKQ6K);
  const int *x_sc = reinterpret_cast<const int *>(x_df + kMmqY);
  const int *y_qs = y + 4;
...
#pragma unroll 1
  for (int k01 = 0; k01 < kMmqTileNeK; k01 += 4) {
    const int k0 = k00 + k01;

    TileA a[ntx];
#pragma unroll
    for (int n = 0; n < ntx; ++n) {
      m::LoadGeneric(reinterpret_cast<TileLoad &>(a[n]),
                     x_qs + (i0 + n * TileA::I) * kMmqMmaTileXKQ6K + k0,
                     kMmqMmaTileXKQ6K);
    }
...
#pragma unroll
    for (int j0 = 0; j0 < mmq_x; j0 += ntx * TileC::J) {
      TileB b[1];
      m::LoadGeneric(reinterpret_cast<TileLoad &>(b[0]),
                     y_qs + j0 * kMmqTileYK + k01, kMmqTileYK);

      const int j = j0 + TileC::get_j(0);
      const float dB = y_df[j * kMmqTileYK + k01 / 32] / 2;
...
#pragma unroll
        for (int l = 0; l < TileC::ne; ++l) {
          ...
          const int8_t *sc =
              reinterpret_cast<const int8_t *>(x_sc + i * kMmqMmaTileXKQ6K +
                                              k00 / 16);
          sum[(j0 / TileC::J + n) * TileC::ne + l] +=
              c.x[l] * sc[k01 / 4] * x_df[i * kMmqMmaTileXKQ6K] * dB;
        }
      }
    }
  }
...
__global__ void __launch_bounds__(kMmqWarps * 32, 1)
  ...
  int *tile_y = smem;
  int *tile_x = tile_y + mmq_x * kMmqTileYK;
  ...
  const int *y = reinterpret_cast<const int *>(act) +
                 static_cast<size_t>(jt) * mmq_x *
                     (sizeof(BlockQ8_1Mmq) / sizeof(int));

  for (int kb0 = 0; kb0 < blocks_per_row; kb0 += blocks_per_iter) {
    LoadTilesQ6KMma<kMmqY>(x, tile_x, kb0, tile_x_max_i, blocks_per_row);
    {
      const int *by0 = y + kMmqY * (kb0 * QK / 128) * sz;
      // Stage mmq_x activation tiles into tile_y.
      const int *src = by0;
#pragma unroll
    ...
    __syncthreads();
    // vec_dot processes one 32-K slice at a time over the 256-elem
    // super-block range covered by this iteration.
#pragma unroll
    for (int kk = 0; kk < blocks_per_iter * (QK / kMmqTileNeK); ++kk) {
      VecDotQ6KQ8_1Mma<mmq_x>(tile_x, tile_y, sum, kk * kMmqTileNeK);
    }
    __syncthreads();
  }
