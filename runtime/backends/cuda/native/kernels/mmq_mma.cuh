...
static __global__ void SiluMulQuantizeQ8_1MmqKernel(
    const half *__restrict__ gate, const half *__restrict__ up,
    BlockQ8_1Mmq *__restrict__ y, int K, int total_rows) {
  const int row = blockIdx.y;
  if (row >= total_rows)
    return;
  const int t = threadIdx.x; // 0..127
...
template <int mmq_y>
__device__ __forceinline__ void
LoadTilesQ6KMma(const char *__restrict__ x, int *__restrict__ x_tile, int kbx0,
...
#pragma unroll
  for (int i0 = 0; i0 < mmq_y; i0 += kMmqMmaWarps * rows_per_warp) {
    const int i =
        (i0 + threadIdx.y * rows_per_warp + threadIdx.x / (kMmqTileNeK / 8)) %
        mmq_y;
    if (i > i_max) {
      break;
    }
    const block_q6_k *bxi = reinterpret_cast<const block_q6_k *>(x) + kbx0 +
                            i * stride + (threadIdx.x % (kMmqTileNeK / 8)) / 4;
...
#pragma unroll
    for (int k01 = 0; k01 < kMmqTileNeK; k01 += 8) {
      ...
      m::LoadLdmatrix(
          a[n][k01 / 4 + 1],
          reinterpret_cast<const int *>(
              x_qs + (i0 + n * TileA::I) * kMmqMmaTileXKQ6K + (k0 + TileA::J)),
          kMmqMmaTileXKQ6K);
    }
...
#pragma unroll
      for (int l = 0; l < TileC::ne / 2; ++l) {
        const int i = i0 + n * TileC::I + TileC::get_i(2 * l);
        const int sc_packed = x_sc[i * kMmqMmaTileXKQ6K + k0 / 16];
...
#pragma unroll
    for (int k01 = 0; k01 < kMmqTileNeK; k01 += 8) {
      ...
      m::LoadGeneric(b[1], y_qs + j0 * kMmqTileYK + TileB::J + k01, kMmqTileYK);
...
__global__ void __launch_bounds__(kMmqMmaWarps * 32, 1)
    InferfluxMmqQ6KMma(const char *__restrict__ w,
                       const BlockQ8_1Mmq *__restrict__ act,
                       half *__restrict__ out, int N, int K, int M,
                       float *__restrict__ partials, int ksplits) {
  constexpr int warp_size = 32;
  ...
  const char *x =
      w + static_cast<size_t>(it) * kMmqY * blocks_per_row * sizeof(block_q6_k);
