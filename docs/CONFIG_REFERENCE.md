...
| `INFERFLUX_CUDA_ATTN_PACKED_DECODE` | GQA-packed decode attention for head_dim 128 / GQA 8 (warp-owned Q-head pairs, in-register reductions, K/V tiles staged in the cache dtype). Default on (measured +7% avg decode at c=16 vs the fp32-tile split kernel, clean determinism); `0` restores the split-KV kernel. Distinct from `INFERFLUX_CUDA_ATTN_WPH`, a recorded negative result that stays off |
