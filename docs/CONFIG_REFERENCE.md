...
| `INFERFLUX_DISABLE_GREEDY_ARGMAX` | set non-`0`/non-`false` to disable the unified-batch greedy fast path. When temperature ≤ 0 with no penalties and no logit_bias, the llama.cpp wrapper samples by first-max over the raw logits row instead of building llama's full-vocab token_data array per request per step (3.6 MB on a 152k vocab); the selected token is bit-identical. Kill switch for validation only |
