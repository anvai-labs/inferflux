...
| `INFERFLUX_LLAMA_FLASH_ATTENTION` | llama.cpp wrapper FlashAttention override (`true`/`1` to force on, anything else off). Without it, FA follows the YAML: `runtime.cuda.flash_attention.enabled` (CUDA builds only) or `runtime.rocm.flash_attention.enabled` (measured +2.2x decode at c=1 and c=16 on gfx1201 with the default off — ROCm configs should set it). Unsupported targets fall back to no-FA via the backend trait gate |
| `INFERFLUX_LLAMA_CTX_SIZE` | llama.cpp wrapper total context (`n_ctx`) before `n_seq_max` partitioning; per-sequence capacity is `n_ctx_seq = ceil(n_ctx / n_seq_max)` padded to 256, and the KV pool is `n_ctx_seq * n_seq_max`. Default 2048 with the default 128 sequences therefore reserves a 32,768-token pool of 256-token slots — sequences decode-fail (with per-slot eviction now armed) past 256 tokens. Valid range 256-2097152; invalid values warn and keep the default |
