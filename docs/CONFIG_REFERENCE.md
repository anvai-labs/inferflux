...
| `INFERFLUX_CUDA_ATTN_MMA_DECODE` | tensor-core (`mma.m16n8k16`) decode attention for head_dim 128 / GQA 8 / fp16: all GQA heads packed into the mma N dimension, per-warp online softmax over 32-row KV slices, f16 PV accumulation. **Experimental, default off** — kernel-vs-kernel rig (`benchmark_fa_decode_rig`) measures 1.4-3x over the packed kernel across kv 128-1024; `1` enables. Falls back to the packed kernel for bf16 or other shapes |
