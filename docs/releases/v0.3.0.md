...
- Green portable CI and exact-main `Dual-GPU gate result`, per ADR-0005; do not run PR code on
  the persistent GPU runner or bypass a failed/missing runtime gate.
- Tag v0.3.0 only after those gates, then verify Linux x86/arm, macOS, Windows packages,
  manifests, and checksums from the existing release workflow.
