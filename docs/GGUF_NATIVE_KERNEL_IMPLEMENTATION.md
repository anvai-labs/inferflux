...
| Grouped projections | Q/K/V triple and gate/up pair launches share a single fused RmsNorm+Quantize kernel followed by grouped Q8_1 GEMV (one kernel for 2-3 outputs). The single-row `q8_1_group_hot_q4k` path remains the `M=1` default. For `Q4_K M>=2`, `q8_1_group_mmq3` is preferred: on Ada RTX 4000 the exact `M=2,N=11008,K=2048` path measured 0.105 ms versus 0.146 ms for fused gate/up and passed 44,050 row-pair parity assertions. Disabling MMQ3 retains `q8_1_group_row_pair_w4` as the `M=2` fallback. |
...
| Q6_K Ada decode | Vectorized Q6_K extraction is automatic for `M<=8` on SM 8.9, with `INFERFLUX_DISABLE_Q6K_VECTORIZED=1` as a rollback. RTX 4000 Ada validation measured five-run median speedups of 1.45-1.97x across M=3/4/5/8; see [the scoped validation record](benchmarks/q6k-vectorized-sm89-2026-08-30.md). Other GPU profiles retain the conservative default unless `INFERFLUX_ENABLE_Q6K_VECTORIZED=1` is set. |
