...
| P2 | gpt-oss tool-calling (harmony `commentary` channel) | Not started (Sep 15) | Chat template rendering + reasoning/content separation for gpt-oss are implemented and tested (`RenderHarmony`, `HarmonySplitter`); the `commentary` channel (tool-call recipient/arguments) is deliberately discarded, not surfaced — no tool-calling support for gpt-oss models yet |
| P2 | Parity-delegate VRAM budget starves itself | New (Sep 14) | The native scaffold loads a full duplicate of the GGUF weights, then the delegate's lazy load re-reads the same GGUF; on a 20 GB card with a 3B f16 sidecar the pre-check correctly skips the delegate (4.7 GB free < 5.9 GB needed), leaving structured output honestly unavailable on native providers. Fix shape: scaffold should load vocab/weights-only (no KV, shared or no second weight copy), or the delegate should reuse the scaffold's model handle. |
