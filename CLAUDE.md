...
**Key CUDA env vars:** (centralized in `NativeExecutionPolicy::FromEnv()` in `native_execution_policy.h` — read that file for the full, current list; the parsed default is shown in each `ParseBoolEnv` call)
- `INFERFLUX_ENABLE_BATCHED_DECODE=0` — opt out of batched decode (default on)
- `INFERFLUX_DISABLE_CUDA_GRAPH=1` — disable CUDA graph capture (graphs are enabled by default; a `cudaDeviceSynchronize` before capture prevents heap corruption)
- `INFERFLUX_DISABLE_Q8_1_ACTIVATIONS=1` / `INFERFLUX_DISABLE_PREPACKED_ACTIVATIONS=1` — disable pre-quantized activation paths
...
**InferFlux CUDA kernel files** (`runtime/backends/cuda/native/kernels/`):
- `mmvq.cuh` — MMVQ weight-read-first kernels (batch 1–8, primary dispatch path)
- `mmq.cuh` / `mmq_grouped_ffn.cuh` — MMQ tiled quantized GEMM kernels (batch 9–64) and grouped FFN variants
- `fused_dequant_gemv.cuh` — V1 GEMV kernels (column-major, 8 warps/block, pair/triple M>8 fallback)
- `dequantization.cu` / `.cuh` — standalone dequant paths
- `quant_common.cuh` — shared quantization primitives (Dp4aS8, Vsubss4, LoadPacked*)
...
- `runtime/backends/cuda/native/gpu_sampler.cu`, `kv_cache_gpu.cu`, `cublas_gemm.cpp`
