...
TEST_CASE("Decode relay fingerprint matches identical batch", "[decode_relay]") {
...
TEST_CASE("Decode relay fingerprint rejects different sequence ids",
  ...
  REQUIRE_FALSE(
      DecodeRelayIdentityMatches(armed, {5, 7}, {0, 0}, {29, 30}, 2));
  REQUIRE_FALSE(
      DecodeRelayIdentityMatches(armed, {6, 5}, {0, 0}, {29, 30}, 2));
}
...
TEST_CASE("Decode relay fingerprint rejects slot-reuse generation change",
  ...
  REQUIRE_FALSE(
      DecodeRelayIdentityMatches(armed, {5, 6}, {0, 1}, {29, 30}, 2));
}

TEST_CASE("Decode relay fingerprint rejects n_past mismatch", "[decode_relay]") {
  const auto armed = Armed({5, 6}, {0, 0}, {30, 31});
  // One row not at the position the relay advanced it to.
  REQUIRE_FALSE(
      DecodeRelayIdentityMatches(armed, {5, 6}, {0, 0}, {29, 31}, 2));
  REQUIRE_FALSE(
      DecodeRelayIdentityMatches(armed, {5, 6}, {0, 0}, {30, 30}, 2));
}

TEST_CASE("Decode relay fingerprint rejects size change", "[decode_relay]") {
  const auto armed = Armed({5, 6}, {0, 0}, {30, 31});
  REQUIRE_FALSE(DecodeRelayIdentityMatches(armed, {5, 6, 7}, {0, 0, 0},
                                           {29, 30, 31}, 3));
