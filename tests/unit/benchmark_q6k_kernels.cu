  ...
  // ===================================================================
  {
    ...
    for (int M : {1, 8, 16}) {
      ...
      for (int ks : {1, 2, 3, 4, 6}) {
        ...
        g.z = ks;
        for (int i = 0; i < kWarmup; ++i) {
          native::InferfluxMmqQ6KMma<16><<<g, mma_block, smem, s>>>(
          ...
          for (int m = 0; m < M; ++m) {
            ...
            for (int blk = 0; blk < kBlocksPerRow; ++blk) {
              ...
              for (int e = 0; e < 256; ++e) {
                ...
                const int ql_val =
                    (sub >= 2) ? (b.ql[ql_idx] >> 4) : (b.ql[ql_idx] & 0xF);
                const int h = (b.qh[g2 * 32 + l2] >> (sub * 2)) & 0x03;
                const int q = ((ql_val | (h << 4)) - 32);
                const float sc =
                    static_cast<float>(b.scales[g2 * 8 + sub * 2 + l2 / 16]);
            ...
            const float got =
                __half2float(out[static_cast<size_t>(m) * kN + n]);
            const double rel = std::fabs(ref) > 1e-9
