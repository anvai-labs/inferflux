...
std::vector<half> RunKernel(const std::vector<block_q6_k> &w,
  ...
  // Gate 2: device quantizer vs host emulation (group-major compare).
  {
    ...
    QuantizeRowQ8_1MmqKernel<<<qgrid, 128, 0, buf.s>>>(buf.acts, buf.a, K, M);
    ...
    Report("quantizer == host emulation",
           std::memcmp(dev.data(), gm.data(),
                       dev.size() * sizeof(BlockQ8_1Mmq)) == 0,
           0.0);
  }
...
void TestShape(int M, int N, int K, int ksplits, uint32_t seed) {
  ...
  for (auto &b : w) {
    for (int i = 0; i < 128; ++i)
      b.ql[i] = Lcg(seed) & 0xFF;
    for (int i = 0; i < 64; ++i)
      b.qh[i] = Lcg(seed) & 0xFF;
    for (int i = 0; i < 16; ++i)
      b.scales[i] = 1 + (Lcg(seed) % 15);
    const half d = __float2half(0.001f + 0.004f * (Lcg(seed) % 1000) / 1000.0f);
    std::memcpy(&b.d, &d, 2);
  }
  ...
  const std::vector<half> out =
      RunKernel(w, acts, row_major, M, N, K, ksplits, buf);
  ...
  for (int col = 0; col < N; ++col) {
    for (int j = 0; j < M; ++j) {
      ...
      for (int kb = 0; kb < blocks_per_row; ++kb) {
        const block_q6_k &b = w[static_cast<size_t>(col) * blocks_per_row + kb];
        for (int e = 0; e < 256; ++e) {
...
void TestUniform() {
  ...
  for (size_t i = 0; i < out.size(); ++i) {
    if (__half2float(out[i]) != expect)
      ++n_bad;
    max_dev = std::max(
        max_dev, static_cast<double>(std::fabs(__half2float(out[i]) - expect)));
  }
