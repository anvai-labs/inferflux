...
int main() {
  ...
  for (auto &b : w) {
    for (int i = 0; i < 128; ++i) b.ql[i] = (i * 7) & 0xFF;
    for (int i = 0; i < 64; ++i) b.qh[i] = (i * 11) & 0xFF;
    for (int i = 0; i < 16; ++i) b.scales[i] = 1; // scale 1
    const half d = __float2half(0.01f);
    std::memcpy(&b.d, &d, 2);
  }
  std::vector<half> acts(K);
  for (int i = 0; i < K; ++i) acts[i] = __float2half(0.001f * i);
  ...
  for (int sub = 0; sub < 4; ++sub) {
    ...
    for (int i = 0; i < 32; ++i)
      a[0].qs[sub*32+i] = (int8_t)std::lround(__half2float(acts[sub*32+i])*d_inv);
  }
  block_q6_k *d_w; BlockQ8_1Mmq *d_a; half *d_out;
  cudaMalloc(&d_w, w.size()*sizeof(block_q6_k));
  cudaMalloc(&d_a, a.size()*sizeof(BlockQ8_1Mmq));
  cudaMalloc(&d_out, M*N*sizeof(half));
  cudaMemcpy(d_w, w.data(), w.size()*sizeof(block_q6_k), cudaMemcpyHostToDevice);
  cudaMemcpy(d_a, a.data(), a.size()*sizeof(BlockQ8_1Mmq), cudaMemcpyHostToDevice);
  dim3 grid((N+kMmqY-1)/kMmqY, (M+15)/16);
  const size_t smem = (16*kMmqTileYK + kMmqY*kMmqMmaTileXKQ6K)*sizeof(int);
  printf("grid=(%d,%d) smem=%zu\n", grid.x, grid.y, smem);
  auto err = cudaFuncSetAttribute(InferfluxMmqQ6KMma<16>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smem);
  printf("setattr: %s\n", cudaGetErrorString(err));
  InferfluxMmqQ6KMma<16><<<grid, 256, smem>>>(
      (const char*)d_w, d_a, d_out, N, K, M);
