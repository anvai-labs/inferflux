...
int main() {
  ...
  InferfluxMmqQ6KMma<16><<<grid, 256, smem>>>((const char *)d_w, d_a, d_out,
                                               N, K, M);
  ...
  printf("out[0]=%.2f expect=%.2f ratio=%.4f\n", __half2float(out[0]),
         expect, __half2float(out[0]) / expect);
  {
    ...
    for (size_t idx = 0; idx < out.size(); ++idx) {
      ...
      else { ++other; if (!first_other) first_other = v; }
    }
    printf("hist: exact=%d zero=%d other=%d (first other=%.1f) of %zu\n",
           exact, zero, other, first_other, out.size());
  }
  return 0;
}
