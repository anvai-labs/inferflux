...
__global__ void ident_test(int *out) {
  ...
  for (int i = tid; i < 64; i += 64) a_s[i] = 0x01010101;
  for (int i = tid; i < 32; i += 64) b_s[i] = 0x01010101;
  __syncthreads();
  if (threadIdx.y != 0) return;  // single warp computes
  m::Tile<16, 4, int> a;
  m::Tile<8, 4, int> b;
  m::Tile<16, 8, int> c;
...
int main() {
  cudaFree(0);
  int *d; cudaMalloc(&d, 16 * 8 * sizeof(int));
  ...
  int bad = 0;
  for (int i = 0; i < 128; ++i)
    if (h[i] != 4) { if (bad < 6) printf("C[%d][%d]=%d (want 4)\n", i/8, i%8, h[i]); ++bad; }
